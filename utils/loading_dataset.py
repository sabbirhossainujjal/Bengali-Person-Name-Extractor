"""This screpts reads files from each of the dataset and returns data in dataframe format with columns =['sentences', 'labels']
...
def readfile_dataset_1(filename):
...
def readfile_dataset_2(filename):
    """
...
def readfile(filename, dataset= 2):
    if dataset == 1:
        df= readfile_dataset_1(filename= filename)
    else:
        df = readfile_dataset_2(filename= filename)
    return df